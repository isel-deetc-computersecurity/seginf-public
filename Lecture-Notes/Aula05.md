...
Perceba, ainda, que a **resistência a colisão implica resistência a segunda pre-imagem**. Dito de outra forma: se, dado um $x$ é fácil encontrar um $x'$ que compartilha a mesma imagem, então, para encontrar um par qualquer $(x, x')$ de pré-imagens que compartilhem uma mesma imagem, basta escolhermos um $x$ arbitrário.
