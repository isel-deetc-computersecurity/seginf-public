...
Outras aplicações particulares podem se beneficiar de mecanismos de **correção de erros**. Isso porque um criptograma armazenado em um dispositivo de memória ou transmitido por um *link* de comunicação pode sofrer corrupções de diversas naturezas, incluindo a alteração do valor de certos bits, a remoção de certos bits ou a adição de outros bits. Se não tratadas, estas corrupções tendem a ser amplificadas pelo processo de decifra, resultando em uma mensagem $m'$ ainda mais distante da mensagem original $m$.
